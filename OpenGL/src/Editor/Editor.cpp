...
void Editor::RunImGuiFrame()
{
	...
	if (ImGui::Button("Play"))
	{
		...
		else
		{
			...
			//SceneSerializer ser;
			//ser.DeserializeYAML(scenePtr, "saves/scene.scene");
		}
	}
	else
	{
		if (ImGui::Button("Save"))
		{
			...
			{
				SceneSerializer ser;
				ser.SerializeYAML(scenePtr, "saves/scene.scene");
			}
		}
...
void Editor::LoadEditorScene(Scene* scene)
{
	...
	std::unique_ptr<GameObject> CameraObject(new GameObject("CameraObject"));
	CameraObject->SetTransformPosition(0, 1.5f, 0);
	std::unique_ptr<Camera> camera(new Camera(aspectRatio, viewWidth));//, "Camera"));
	camera->SetGameObject(CameraObject.get());
	camera->GetGameObject()->GetTransfrom()->SetPosition(0, 1, 5);
	camera->GetGameObject()->GetTransfrom()->SetRotation(0,0,0);
	std::unique_ptr<MovementController> mv(new MovementController());
	//CameraObject->AddScript(std::move(mv));
	scene->AddGameObjectToScene(std::move(CameraObject));
	scene->camera = std::move(camera);

	scene->topGameObjects[3]->AddScript(std::move(mv));
	...
	//Assets
	//Grab shaders from assests then bind and set uniforms
	//Shaders
	...
	SceneSerializer::SerializeMaterial("res/material/DefaultMaterial.mat", defaultMaterial.get());

	std::unique_ptr<Material> material(new Material(std::move(shader), std::move(texture)));
	SceneSerializer::SerializeMaterial("res/material/material.mat", material.get());

	std::unique_ptr<Material> playerMaterial(new Material(std::move(playerShader), std::move(playerTexture)));
	SceneSerializer::SerializeMaterial("res/material/playerMaterial.mat", playerMaterial.get());

	std::unique_ptr<Material> floorMaterial(new Material(std::move(floorShader)));
	SceneSerializer::SerializeMaterial("res/material/floorMaterial.mat", floorMaterial.get());

	std::unique_ptr<Material> wallMaterial(new Material(std::move(wallShader)));
	SceneSerializer::SerializeMaterial("res/material/wallMaterial.mat", wallMaterial.get());
	//End Materials
	...
	SceneSerializer::SerializeMesh("res/mesh/DefaultMesh.mesh", defaultMesh.get());
	
	std::unique_ptr<Mesh> mesh(new Mesh());
	mesh->ImportModelFromTxtFile("res/models/Square.txt");
	mesh->SetMaterial(std::move(material));
	SceneSerializer::SerializeMesh("res/mesh/mesh.mesh", mesh.get());
	...
	SceneSerializer::SerializeMesh("res/mesh/playerMesh.mesh", playerMesh.get());
	...
	SceneSerializer::SerializeMesh("res/mesh/floorMesh.mesh", floorMesh.get());
	...
	SceneSerializer::SerializeMesh("res/mesh/wallMesh.mesh", wallMesh.get());
	//End Mesh
	//End Assets
...
int Editor::StartEditor()
{
	...
	{	
		...
		LoadEditorScene(scenePtr);//TODO: Put this into derserializeYaml
