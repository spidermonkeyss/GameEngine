	...
	for (int i = 0; i < gameObjects.size(); i++)
	{
		if (gameObjects[i]->GetMesh() != nullptr)
		{
			if (gameObjects[i]->GetMesh()->enabled)
			{
				...
				model[0].x = gameObjects[i]->GetTransfrom()->GetScale().x;
				model[1].y = gameObjects[i]->GetTransfrom()->GetScale().y;
				model[2].z = gameObjects[i]->GetTransfrom()->GetScale().z;

				glm::mat4 mvp = renderer->GetProjMat4() * glm::inverse(renderer->GetCamera()->GetGameObject()->GetTransfrom()->GetMat4()) * model;
